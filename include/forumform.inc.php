...
// $Id: forumform.inc.php,v 1.6 2006/03/10 17:08:37 mikhail Exp $
//  ------------------------------------------------------------------------ //
...
echo "<form action='post.php' method='post' name='forumform' id='forumform' onsubmit='return xoopsValidate(\"subject\", \"message\", \"contents_submit\", \"" . htmlspecialchars(_PLZCOMPLETE, ENT_QUOTES) . '", "' . htmlspecialchars(_MESSAGETOOLONG, ENT_QUOTES) . '", "' . htmlspecialchars(
    _ALLOWEDCHAR,
    ENT_QUOTES
) . '", "' . htmlspecialchars(_CURRCHAR, ENT_QUOTES) . "\");'><table cellspacing='1' class='outer' width='100%'><tr><td class='head' width='25%' valign='top'>" . _MD_ABOUTPOST . ':</td>';
...
if ($xoopsUser && 2 == $forumdata['forum_access'] && !empty($post_id)) {
